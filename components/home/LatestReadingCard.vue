<template>
  <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 flex flex-col justify-between h-full">
    <div>
      ...
      <div v-else-if="latestReading">
        <a :href="`/books/${latestReading.bookid}.html`">
          <img class="rounded-t-lg w-full" :src="`/covers/${latestReading.bookid}.webp`" :alt="latestReading.title" />
        </a>
        <br />
        <div class="space-y-2 dark:text-gray-300">
          <p class="font-semibold text-lg">{{ latestReading.reviewtitle }}</p>
          <p class="text-sm">
            评论书籍：
            <a :href="`/books/${latestReading.bookid}.html`" class="font-semibold text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
              《{{ latestReading.title }}》
            </a>
          </p>
          <p class="text-sm text-gray-600 dark:text-gray-400">
            作者：{{ latestReading.author }}
          </p>
          <p class="text-xs text-gray-500 dark:text-gray-500">
            发表于：{{ formatDate(latestReading.create_at) }}
          </p>
          <p class="text-xs text-gray-500 dark:text-gray-500">
            共有 {{ latestReading.reviews_count }} 篇书评
          </p>
        </div>
      </div>
    </div>
    
    <a v-if="latestReading" :href="`/books/${latestReading.bookid}.html`"
...
// TODO: the displayed data is not what I wanted. Will look soon.
